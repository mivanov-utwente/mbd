...
def partyFlatMap(x):
  ...
  for k, party in pol_parties.items():
    other_party_rules = [rule for ok, op in pol_parties.items() if ok != k for rule in op['rules']]
    if any(rule.search(x['text']) for rule in party['rules']) and \
       not any(rule.search(x['text']) for rule in other_party_rules):
      result.append((k, x['description_score'] +  x['text_score']))
  return result
...
def formatRdd(rdd):
  return rdd.map(lambda x: '"{}",{},{},{},{}'.format(unicode(x[0]).encode("utf8"), *x[1]))
